...
public class DAO extends DBContext {
    ...
    public void insert(User c) {
        String sql = "INSERT INTO [dbo].[User_HE163884]\n"
                + "           ([password]\n"
                + "           ,[name]\n"
                + "           ,[email]\n"
                + "           ,[sex]\n"
    ...
    public void insert(Movie c) {
        String sql = "INSERT INTO [dbo].[Movie_HE163884]\n"
                + "           ([FilmID]\n"
                + "           ,[HotLevel]\n"
                + "           ,[Information]\n"
                + "           ,[Price]\n"
    ...
    public List<String> getListDay() {
        String sql = "SELECT distinct day\n"
                + "  FROM [PRJ301_SE1648].[dbo].[Schedule_HE163884]";
    ...
    public void insertFilmWithSchedule(Movie e) {
        try {
            ...
            String sql1 = "INSERT INTO [dbo].[Movie-Schedule_HE163884]\n"
                    + "     VALUES\n"
                    + "           (?\n"
                    + "           ,?)";
    ...
    public User getUserByName(String name) {
        String sql = "select * from [dbo].[User_HE163884] where name=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, name);
            ResultSet rs = st.executeQuery();
    ...
    public User getUserById(int id) {
        String sql = "select * FROM [dbo].[User_HE163884] where UserID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, id);
            ResultSet rs = st.executeQuery();
    ...
    public Movie getMovieById(String id) {
        String sql = "select * FROM [dbo].[Movie_HE163884] where FilmID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, id);
            ResultSet rs = st.executeQuery();
            ...
            if (rs.next()) {
                ...
                String sql1 = "select * from [dbo].[Movie-Schedule_HE163884] where FilmID=?";
                try {
                    PreparedStatement st1 = connection.prepareStatement(sql1);
                    st1.setString(1, id);
...
//                        s.setStartTime(rs1.getString("StartTime"));
                ...
                c.setSchedules(list);
                int male = 0, female = 0;
                String sqls = "select distinct UserID,FilmID,SID from bill_HE163884 where FilmID=?";
    ...
    public Schedule getScheduleById(int id) {
        String sql = "select * from [dbo].[Schedule_HE163884] where SID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, id);
            ResultSet rs = st.executeQuery();
    ...
    public List<Movie> getAllMovieNotPublish() {
        ...
        String sql = "select * from Movie_HE163884 where [Publish_date]>?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
    ...
    public Schedule getSchedule(String starttime, String day) {
        String sql = "select * from [dbo].[Schedule_HE163884] where day=? and StartTime=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
    ...
    public List<Schedule> getAllSchedule() {
        List<Schedule> list = new ArrayList<>();
        String sql = "select * from [dbo].[Schedule_HE163884] where day='2022-07-17'";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public void update(Movie c) {
        String sql = "UPDATE [dbo].[Movie_HE163884]\n"
                + "   SET [HotLevel] = ?\n"
                + "      ,[Information] = ?\n"
                + "      ,[Price] = ?\n"
                + "      ,[Publish_date] = ?\n"
    ...
    public void update(User c) {
        String sql = "UPDATE [dbo].[User_HE163884]\n"
                + "   SET [name] = ?\n"
                + "      ,[email] = ?\n"
                + "      ,[address] = ?\n"
                + "      ,[phonenumber] = ?\n"
    ...
    public int getUserIDByUserAndPass(String user, String pass) {
        String sql = "select * FROM [dbo].[User_HE163884] where name=? and password=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, user);
            st.setString(2, pass);
    ...
    public List<Movie> search(String name) {
        List<Movie> list = new ArrayList<>();
        String sql = "select * FROM [dbo].[Movie_HE163884] where 1=1 ";
    ...
    public void addSeat() {
        try {
            for (int i = 1; i <= 8; i++) {
                String sql = "INSERT INTO [dbo].[seat_HE163884]\n"
                        + "           ([Position])\n"
                        + "     VALUES\n"
                        + "           ('F" + i + "')";
                PreparedStatement st = connection.prepareStatement(sql);
    ...
    public void addCart(Cart c) {
        try {
            String sql = "INSERT INTO [dbo].[cart_HE163884]\n"
                    + "           ([UserID]\n"
                    + "           ,[FilmID]\n"
                    + "           ,[SID]\n"
                    + "           ,[Position]\n"
    ...
    public void addSeatSchedule(Movie m) {
        try {
            String sql = "SELECT [SID]\n"
                    + "  FROM [dbo].[Schedule_HE163884]";
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                int sid = rs.getInt("SID");
                String sql1 = "SELECT [Position] FROM [dbo].[seat_HE163884]";
                PreparedStatement st1 = connection.prepareStatement(sql1);
                ResultSet rs1 = st1.executeQuery();
                while (rs1.next()) {
                    String seat = rs1.getString("Position");
                    String sql2 = "INSERT INTO [dbo].[seatSchedule_HE163884]\n"
                            + "     VALUES\n"
                            + "           (?\n"
                            + "           ,?\n"
                            + "           ,?\n"
    ...
    public List<Cart> getCartById(int id) {
        ...
        String sql = "select distinct SID,FilmID from cart_HE163884 where UserID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, id);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                ...
                String sql1 = "select * from cart_HE163884 where UserID=? and SID=? and FilmID=?";
                PreparedStatement st1 = connection.prepareStatement(sql1);
                st1.setInt(1, id);
                st1.setInt(2, sid);
    ...
    public Cart getCartByOID(int oid) {
        String sql = "select * from cart_HE163884 where OID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, oid);
            ResultSet rs = st.executeQuery();
    ...
    public void deleteCart(int uid, int sid, String fid) {
        String sql = "DELETE FROM [dbo].[cart_HE163884]\n"
    ...
    public boolean getStatusPosition(int sid, String posit, String film) {
        String sql = "SELECT *\n"
                + "  FROM [PRJ301_SE1648].[dbo].[seatSchedule_HE163884] where SID=? and Position=? and FilmID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, sid);
    ...
    public void makeOrder(int uid) {
        ...
        for (Cart i : list) {
            ...
            for (String p : i.getPositions()) {
                String sql = "UPDATE [dbo].[seatSchedule_HE163884]\n"
                        + "   SET [Status] = 0\n"
                        + " WHERE SID=? and Position=? and FilmID=?";
                String sql1 = "INSERT INTO [dbo].[bill_HE163884]\n"
                        + "           ([UserID]\n"
                        + "           ,[FilmID]\n"
                        + "           ,[SID]\n"
                        + "           ,[Position]\n"
    ...
    public List<String> getPositionBooked(int sid, String film) {
        List<String> list = new ArrayList<>();
        String sql = "SELECT *\n"
                + "  FROM [PRJ301_SE1648].[dbo].[seatSchedule_HE163884] where SID=? and FilmID=? and Status=0";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, sid);
            st.setString(2, film);
    ...
    public void deleteFilm(String id) {
        String sql1 = "DELETE FROM [dbo].[seatSchedule_HE163884]\n"
                + "      WHERE FilmID=?";
        String sql2 = "DELETE FROM [dbo].[Movie-Schedule_HE163884]\n"
                + "      WHERE FilmID=?";
        String sql = "DELETE FROM [dbo].[Movie_HE163884]\n"
                + "      WHERE FilmID=?";
        try {
            PreparedStatement st1 = connection.prepareStatement(sql1);
            st1.setString(1, id);
    ...
    public float getTurnoverOfMonth() {
        float ans = 0;
        String sql = "select distinct UserID,FilmID,SID,price from bill_HE163884 where time>='2022-7-1' and time<='2022-7-30'";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public List<Movie> getMovieByWeek() {
        ...
        List<Movie> ans = new ArrayList<>();
        for (Movie i : list) {
            String sql = "select distinct UserID,FilmID,SID,price from bill_HE163884 where FilmID=? and time>='2022-07-10' and time<='2022-07-16'";
            try {
                PreparedStatement st = connection.prepareStatement(sql);
                st.setString(1, i.getFilmID());
                ResultSet rs = st.executeQuery();
    ...
    public List<Schedule> getStatSchedule() {
        ...
        try {
            for (int i = 1; i <= 5; i++) {
                ...
                String sql = "select count(SID%10)as [Time] from bill_HE163884 where SID%10=?";
                PreparedStatement st = connection.prepareStatement(sql);
                st.setInt(1, i);
                ResultSet rs = st.executeQuery();
                if (rs.next()) {
    ...
    public List<User> getAllUser()
    {
        ...
        String sql="SELECT *  FROM [dbo].[User_HE163884]";
        try{
            PreparedStatement st=connection.prepareStatement(sql);
            ResultSet rs=st.executeQuery();
            while (rs.next())
    ...
    public List<User> getUserCost()
    {
        ...
        try{
            for (User i: list)
        {
            String sql="select distinct UserID,FilmID,SID,price from bill_HE163884 where UserID=?";
            PreparedStatement st=connection.prepareStatement(sql);
    ...
    public List<Movie> getMovieByMonth() {
        ...
        List<Movie> ans = new ArrayList<>();
        for (Movie i : list) {
            String sql = "select distinct UserID,FilmID,SID,price from bill_HE163884 where FilmID=? and time>='2022-07-01' and time<='2022-07-31'";
            try {
                PreparedStatement st = connection.prepareStatement(sql);
                st.setString(1, i.getFilmID());
                ResultSet rs = st.executeQuery();
